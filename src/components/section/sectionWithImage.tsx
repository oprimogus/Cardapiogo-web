...
export function SectionWithImage({
	items,
}: Readonly<{ items: SectionWithImageItem[] }>) {
			...
			{items.length > 0 &&
				items.map((item, index) => {
					return (
						<section
							key={item.textTop}
							className={`px-6 py-12 mx-auto flex flex-col items-center justify-center ${isEven(index) ? "bg-gray-100" : "bg-white"}`}
						>
							...
							<img src={item.imagePath} alt="" className=" w-2/5 h-2/5 m-4" />
